...
class ShouldMakeSuperMarioBros(ShouldMakeEnv, TestCase):
    ...
    env_id = [f"SuperMarioBros-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBrosLostLevels(ShouldMakeEnv, TestCase):
    ...
    time = 400
    # the environments ID for all VERSIONS of Super Mario Bros
    env_id = [f"SuperMarioBros2-{v}" for v in ["Vanilla", "Downsample"]]
...
class ShouldMakeSuperMarioBros_1_1(ShouldMakeEnv, TestCase):
    ...
    stage = 1
    # the environments ID
    env_id = [f"SuperMarioBros-1-1-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_1_2(ShouldMakeEnv, TestCase):
    ...
    stage = 2
    # the environments ID
    env_id = [f"SuperMarioBros-1-2-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_1_3(ShouldMakeEnv, TestCase):
    ...
    stage = 3
    # the environments ID
    env_id = [f"SuperMarioBros-1-3-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_1_4(ShouldMakeEnv, TestCase):
    ...
    stage = 4
    # the environments ID
    env_id = [f"SuperMarioBros-1-4-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_2_1(ShouldMakeEnv, TestCase):
    ...
    stage = 1
    # the environments ID
    env_id = [f"SuperMarioBros-2-1-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_2_2(ShouldMakeEnv, TestCase):
    ...
    stage = 2
    # the environments ID
    env_id = [f"SuperMarioBros-2-2-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_2_3(ShouldMakeEnv, TestCase):
    ...
    stage = 3
    # the environments ID
    env_id = [f"SuperMarioBros-2-3-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_2_4(ShouldMakeEnv, TestCase):
    ...
    stage = 4
    # the environments ID
    env_id = [f"SuperMarioBros-2-4-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_3_1(ShouldMakeEnv, TestCase):
    ...
    stage = 1
    # the environments ID
    env_id = [f"SuperMarioBros-3-1-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_3_2(ShouldMakeEnv, TestCase):
    ...
    stage = 2
    # the environments ID
    env_id = [f"SuperMarioBros-3-2-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_3_3(ShouldMakeEnv, TestCase):
    ...
    stage = 3
    # the environments ID
    env_id = [f"SuperMarioBros-3-3-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_3_4(ShouldMakeEnv, TestCase):
    ...
    stage = 4
    # the environments ID
    env_id = [f"SuperMarioBros-3-4-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_4_1(ShouldMakeEnv, TestCase):
    ...
    stage = 1
    # the environments ID
    env_id = [f"SuperMarioBros-4-1-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_4_2(ShouldMakeEnv, TestCase):
    ...
    stage = 2
    # the environments ID
    env_id = [f"SuperMarioBros-4-2-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_4_3(ShouldMakeEnv, TestCase):
    ...
    stage = 3
    # the environments ID
    env_id = [f"SuperMarioBros-4-3-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_4_4(ShouldMakeEnv, TestCase):
    ...
    stage = 4
    # the environments ID
    env_id = [f"SuperMarioBros-4-4-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_5_1(ShouldMakeEnv, TestCase):
    ...
    stage = 1
    # the environments ID
    env_id = [f"SuperMarioBros-5-1-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_5_2(ShouldMakeEnv, TestCase):
    ...
    stage = 2
    # the environments ID
    env_id = [f"SuperMarioBros-5-2-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_5_3(ShouldMakeEnv, TestCase):
    ...
    stage = 3
    # the environments ID
    env_id = [f"SuperMarioBros-5-3-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_5_4(ShouldMakeEnv, TestCase):
    ...
    stage = 4
    # the environments ID
    env_id = [f"SuperMarioBros-5-4-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_6_1(ShouldMakeEnv, TestCase):
    ...
    stage = 1
    # the environments ID
    env_id = [f"SuperMarioBros-6-1-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_6_2(ShouldMakeEnv, TestCase):
    ...
    stage = 2
    # the environments ID
    env_id = [f"SuperMarioBros-6-2-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_6_3(ShouldMakeEnv, TestCase):
    ...
    stage = 3
    # the environments ID
    env_id = [f"SuperMarioBros-6-3-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_6_4(ShouldMakeEnv, TestCase):
    ...
    stage = 4
    # the environments ID
    env_id = [f"SuperMarioBros-6-4-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_7_1(ShouldMakeEnv, TestCase):
    ...
    stage = 1
    # the environments ID
    env_id = [f"SuperMarioBros-7-1-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_7_2(ShouldMakeEnv, TestCase):
    ...
    stage = 2
    # the environments ID
    env_id = [f"SuperMarioBros-7-2-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_7_3(ShouldMakeEnv, TestCase):
    ...
    stage = 3
    # the environments ID
    env_id = [f"SuperMarioBros-7-3-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_7_4(ShouldMakeEnv, TestCase):
    ...
    stage = 4
    # the environments ID
    env_id = [f"SuperMarioBros-7-4-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_8_1(ShouldMakeEnv, TestCase):
    ...
    stage = 1
    # the environments ID
    env_id = [f"SuperMarioBros-8-1-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_8_2(ShouldMakeEnv, TestCase):
    ...
    stage = 2
    # the environments ID
    env_id = [f"SuperMarioBros-8-2-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_8_3(ShouldMakeEnv, TestCase):
    ...
    stage = 3
    # the environments ID
    env_id = [f"SuperMarioBros-8-3-{v}" for v in VERSIONS]
...
class ShouldMakeSuperMarioBros_8_4(ShouldMakeEnv, TestCase):
    ...
    stage = 4
    # the environments ID
    env_id = [f"SuperMarioBros-8-4-{v}" for v in VERSIONS]
